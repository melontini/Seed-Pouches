...
public class FlowerPouchItem extends Item {
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        if (!world.isClient) {
            FlowerPouchEntity flowerPouchEntity = new FlowerPouchEntity(user, world);
            flowerPouchEntity.setItem(itemStack);
            flowerPouchEntity.setVelocity(user, user.getPitch(), user.getYaw(), 0.0F, 1.5F, 1.0F);
            world.spawnEntity(flowerPouchEntity);
        }
